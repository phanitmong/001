            ...
            <div class="col-sm-7">
                <form action="{{ route('product.update',$product->id) }}"  method="POST" enctype="multipart/form-data" id="sendform">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            <input type="text" class="form-control"  placeholder="បញ្ចូលកូដទំនិញ" name="code" value="{{$product->code}}" autofocus >
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="" class=" col-sm-4">ឈ្មោះ : <span class="text-danger">*</span></label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" required placeholder="បញ្ចូលឈ្មោះទំនិញ" name="name" value="{{$product->name}}" >
                        </div>
                    </div>
                           ...
                           <select name="unit" id="" class="form-control">
                               <option value="">ខ្នាត</option>
                                @foreach ($unit as $cats)
                                    <option value="{{$cats->id}}" {{$product->unit_id==$cats->id?'selected':''}}>{{$cats->name}}</option>
                                @endforeach
                           </select>
                        </div>
                    </div>
