function addItem()
{
    ...
    if(pid=="" || qty=="" || pcode==undefined)
    {
        ...
        tr += "<td>" + qty + "</td>";
        tr += "<td>" + unit + "</td>";
        tr += "<td><a href='#' onclick='removeItem(this, event)' class='btn btn-danger btn-sm btn-oval'>លុប</a>";
        tr +="&nbsp;<a href='#' data-toggle='modal' data-target='#editModal' onclick='editItem(this, event)' class='btn btn-success btn-sm btn-oval'>កែប្រែ</a></td>";
        tr += "</tr>";
        let trs = $("#data tr");
        if(trs.length>0)
        {
...
{
    evtn.preventDefault();
    var con = confirm('តើអ្នកចង់លុបមែនទេ?');
    if(con)
    {
    ...
    if($("#in_date").val()=="" || $("#warehouse").val()=="" || trs.length<=0)
    {
        ...
        $.ajax({
            ...
            success: function (sms) {
                ...
                if(sms>0)
                {
                    location.href = "/stock-out/detail/" + sms;
                }
                else{
